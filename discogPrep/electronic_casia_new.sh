...
APAC=('Russia' 'Mongolia' 'Kazakhstan');

for i in ${APAC[@]};
do
  x=`echo ${i} | sed "s/_/ /g"`
  printf "Catalogged Electronic Music Releases For Central Asia (Total Per Country)  ${x} ";  grep "::${x}::" ALL_FLATDB | grep '::Electronic' | wc -l
   printf "\t Year Of Greatest Catalogged Release 2000s"; grep "::${x}::" ALL_FLATDB | grep '::Electronic' | grep "^201[0-9]" |  awk '{FS="::"} {print $2}' | grep "^201[0-9\-]*" | sed -e "s/\:\:[A-Za-z0-9*]*//g" | sort -rn | uniq -c | sort -rn | head -1
  printf "\n" 
